    ...
    async def get(self, *, tid: objectid.ObjectId, page: int = 1):
        ...
        if tsdoc:
            ...
            if self.can_show_record(tdoc):
                ...
                rdict = dict(
                    (psdoc["rid"], {"_id": psdoc["rid"]}) for psdoc in psdict.values()
                )
        else:
            attended = False
        # discussion
        ddocs, dpcount, dcount = await pagination.paginate(
            discussion.get_multi(
                self.domain_id,
...
class ContestCodeHandler(base.OperationHandler):
    ...
    async def get(self, *, tid: objectid.ObjectId):
        ...
        for tsdoc in tsdocs:
            for pdetail in tsdoc.get("detail", []):
                rnames[pdetail["rid"]] = "U{}_P{}_R{}".format(
                    tsdoc["uid"], pdetail["pid"], pdetail["rid"]
                )
        ...
        if self.can_show_record(tdoc):
            rdocs = (
                await record.get_user_in_problem_multi(
                    uid, self.domain_id, pdoc["doc_id"]
                )
                .sort([("_id", -1)])
                .limit(10)
                .to_list()
    ...
    async def post(
        ...
        *,
        ...
        try:
            ...
            begin_at = (
                self.timezone.localize(begin_at)
                .astimezone(pytz.utc)
                .replace(tzinfo=None)
            )
        except ValueError:
